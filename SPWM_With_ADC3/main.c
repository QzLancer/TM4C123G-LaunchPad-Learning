...
int main(void)
{
    ...
    ADCSequenceStepConfigure(ADC0_BASE, 0, 0, ADC_CTL_CH7); //ADC采集步序配置：采样序列号0，只采集AIN1
...
{
    PID_Instance->SetVoltage=0.7;    //设定电压
...
float PIDCalculator(PID *PID_Instance,float Voltage)
{
    ...
    IncreaseVoltage=PID_Instance->Kp*(PID_Instance->err-PID_Instance->err_last)+PID_Instance->Ki*PID_Instance->err
                    +PID_Instance->Kd*(PID_Instance->err-2*PID_Instance->err_last+PID_Instance->err_last_next);
    PID_Instance->OutputVoltage+=IncreaseVoltage;
//    if(PID_Instance->OutputVoltage>=3.3)
 //       PID_Instance->OutputVoltage=3.3;
//    if(PID_Instance->OutputVoltage<=-50)
//        PID_Instance->OutputVoltage=-50;
